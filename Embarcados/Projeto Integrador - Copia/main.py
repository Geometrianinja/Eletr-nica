...
def game_loop():
    ...
    target = Circle(400, 400, 40, (255, 130, 130))
    ...
    if monitors and monitor_to_use < len(monitors):
        monitor_info = monitors[monitor_to_use][2]
        left, top, _, _ = monitor_info
        os.environ['SDL_VIDEO_WINDOW_POS'] = f"{left},{top}"
    
    
    screen = pygame.display.set_mode((s_width, s_height), pygame.SCALED | pygame.FULLSCREEN)
    ...
    while running:
        ...
        while True:
            ...
            if controller.get_queue_size() < 5:
                break
        ...
        if not calibration.calibrated:
            ...
            if (target.intersects(pointer)):
                if not vibrating:
                    controller.vibrate_on()
                    vibrating = True
            else:
                if vibrating:
                    controller.vibrate_off()
                    vibrating = False
            target.draw(screen)
            pointer.draw(screen)
